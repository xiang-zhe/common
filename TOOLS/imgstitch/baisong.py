...
if __name__=='__main__':
    ...
    im_src = cv2.imread('7.jpg').astype('float32')

    im_dst = cv2.imread('8.jpg').astype('float32')
    ...
    im_actual_L = cv2.cvtColor(im_src.astype('uint8'),44)[:,:,0] #cv2.cv.CV_BGR2LAB = 44
    im_alpha_L = cv2.cvtColor(im_alpha.astype('uint8'),44)[:,:,0]
    im_poisson_L = cv2.cvtColor(im_res.astype('uint8'),44)[:,:,0]
    '''
