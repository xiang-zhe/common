...
sk.bind(("127.0.0.1", 8886))  # 绑定端口,“127.0.0.1”代表本机地址，8888为设置链接的端口地址
...
accept_data2 = str(accept_data.decode())
print("".join(("接收内容：", accept_data2, "    客户端口：", str(addr[1]))))
send_data = input("输入发送内容：")
#conn.sendall(bytes(send_data))  # 发送内容必须为bytes类型数据，bytes(data, encoding="utf8")用“utf8”格式进行编码
